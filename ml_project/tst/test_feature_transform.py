...
def test_fit_and_transform(transformer, fit_data, transform_input_data):
    ...
    assert transformer.cat_feats == ["legs_count"]
    assert transformer.num_feats == ["age"]

    transformed_data = transformer.transform(transform_input_data).to_numpy()
...
def test_fit_transform(transformer, transform_input_data):
    transformed_data = transformer.fit_transform(transform_input_data).to_numpy()
    expected_transform_output_data = np.array([
        [0, 1, -1.33630621],
        [1, 0, 0.26726124],
        [1, 0, 1.06904497],
